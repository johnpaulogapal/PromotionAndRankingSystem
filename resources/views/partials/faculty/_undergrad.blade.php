<div class="w-full py-10 px-5 h-full flex flex-col items-center gap-y-4 border-dashed border-x-2 border-gray-200">
    ...
    <a href="{{route('undergrad.create')}}" class="py-1 px-2 text-lg uppercase text-white tracking-widest bg-cyan-700 rounded shadow-lg transition ease-in-out delay-150 hover:bg-cyan-600 duration-300 mb-5">
    ...
    @else
        @foreach (auth()->user()->undergrads as $undergrad)
            <div class="w-full mb-2 p-5 border-t-8 {{ $undergrad->status == 'verified' ? 'border-green-700' : 'border-hau'}} rounded-b-lg shadow-2xl space-y-4">
                ...
                @if($undergrad->status == 'verified' || auth()->user()->application->app_status == 'approved')
                ...
                @if(auth()->user()->application->app_status == 'approved')
                    <p></p>
                @else
                    @if(($undergrad->status == 'pending' || $undergrad->status == 'resubmit'))
                        ...
                        <div
                            ...
                            class="min-[576px]:shadow-[0_0.5rem_1rem_rgba(#000, 0.15)] pointer-events-auto relative flex w-full flex-col rounded-md border-none bg-white bg-clip-padding text-current shadow-lg outline-none">
                                ...
                                <div class="relative flex-auto p-4" data-te-modal-body-ref>
                                    <p class="font-bold uppercase tracking-widest">Are you sure you want to delete this Undergrad Information permanently?</p>
                                </div>
                                <div
