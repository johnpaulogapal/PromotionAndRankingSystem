<div class="overflow-x-auto sm:-mx-6 lg:-mx-8">
    ...
    <form method="POST" action="{{route('oces.scoreUpdate', [ 'user' => $user->id, 'score' => $user->score->id])}}" class="overflow-hidden">
    @csrf
    @method('PUT')
        ...
        <tbody>
            ...
            <tr class="border-b dark:border-neutral-500">
              ...
              <td class="whitespace-nowrap px-6 py-4">
                  @if(empty($user->score->com_ser))
                      <div class="flex flex-col justify-center gap-1">
                          <label for="" class="font-bold text-hau text-sm tracking-wider"></label>
                          <input name="com_ser" type="number" class="w-1/4 py-0.5 px-2 border border-hau rounded caret-hau outline-hau" value="{{old('com_ser')}}" min="1" max="10" required>
                          @error('com_ser')
