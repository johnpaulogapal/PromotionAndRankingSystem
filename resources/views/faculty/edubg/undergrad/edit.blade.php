    ...
    <x-faculty.navigation>
        ...
        <section>
            <div class="h-screen w-full pt-20">
                    ...
                    <form method="POST" action="{{route('undergrad.update', $undergrad->id)}}" enctype="multipart/form-data" class="pt-10 mx-5 h-full flex flex-col items-center gap-y-4">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="p-2 my-1 flex flex-col gap-1 text-white bg-orange-700 rounded">
                            ...
                            <p class="font-bold uppercase text-sm">{{$undergrad->comment}}</p>
                        </div>
                        @endif

                        <p class="uppercase text-xl tracking-widest"><span class="py-1 px-2 bg-hau text-white rounded shadow-lg"><i class="fa-solid fa-pen-to-square mr-2"></i>Edit</span> Undergrad Information</p>
                        <div  class="w-full md:w-1/2 py-5 px-10 border-t-2 border-hau rounded-b-lg shadow-2xl space-y-5">
                            ...
                            <div class="flex flex-col justify-start gap-y-1">
                                ...
                                <input name="school" type="text" class="py-0.5 px-2 border border-hau rounded caret-hau outline-hau" value="{{$undergrad->school}}">
                                @error('school')
                            ...
                            <div class="flex flex-col justify-start gap-y-1">
                                ...
                                <input name="course" type="text" class="py-0.5 px-2 border border-hau rounded caret-hau outline-hau" value="{{$undergrad->course}}">
                                @error('course')
                                    <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                                @enderror
                            </div>
                            <div class="flex flex-col justify-start gap-y-1">
                                ...
                                <input name="graduation_date" type="date" class="py-0.5 px-2 border border-hau rounded caret-hau outline-hau" value="{{$undergrad->graduation_date}}">
