<x-faculty.layout>
    <x-faculty.navigation>
        ...
        <section class="h-screen w-full pt-20 p-8">
            ...
            <form method="POST" action="{{route('mpo.store')}}" class="pt-5 grid justify-items-center content-start gap-y-8">
            @csrf
                <div class="w-1/2 p-10 border-t-4 border-hau grid grid-cols-2 gap-4 rounded-b shadow-2xl">
                    ...
                    <div class="grid grid-rows-2 gap-y-4">
                        <div class="flex flex-col justify-start gap-y-1">
                            ...
                            <input name="org_name" type="text" class="py-0.5 px-2 border border-hau rounded caret-hau outline-hau">
                            @error('prc_num')
                ...
                <div class="mt-5 flex justify-center gap-x-4">
                    <a href="{{route('mpo.index')}}" class="py-1.5 px-4 text-xl text-white tracking-widest bg-red-600 rounded shadow-lg transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 hover:bg-red-700 duration-300">
